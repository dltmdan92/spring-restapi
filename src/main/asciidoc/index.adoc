...
이것을 ascii doc 파일이
프로젝트 빌드되면서 html화 된다.
REST API에서 각 필드에 대한 description을 해당 adoc 링크로 걸어주면
REST의 조건중 하나인 self description이 완성됨
target/docs에 index.html 생성됨 (메이븐 빌드)
...
////
아래에 이벤트 생성을 위한 snippet들을 추가했음
(http-request,request-headers,response-fields)
////

operation::create-event[snippets='request-fields,curl-request,http-request,request-headers,response-fields,links']
